...
Once the package *doubt* is attached a dubious operator can be built from any sequence of characters containing at least one `?` as long as `x <dubious op> y` is syntactic. 

It is defined like any regular function, but using backticks, for instance :
...
In practice we advise to stick to some clear templates to avoid confusion.

To be recognized by *doubt* an operator must be either defined in the global
environment, be defined in a package AND registered (see dedicated section),
or have the standard `?fun?` format, where `fun` is accessible from where `?fun?`
is called.
...
We propose an example withe the *Julia* language, using the package *JuliaCall*.
...
Packages using doubt should either depend on *doubt* or import it and reexport
`?`.

In the future *doubt* should export only `?` so it should be clean to import it,
so we don't have different versions of `?` overriding each other.

In any case these packages should define an `.onAttach` function which will increment the `options("doubt.registered_ops")` with the names of the dubious
operators of the package.
